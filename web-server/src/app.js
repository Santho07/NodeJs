  ...
  geocode(req.query.address, function geoCodeCallback(error, response) {
    ...
    weather(latitude, longitude, function weatherCallback(error, response) {
      ...
      let myObject = {
        ...
        updateTime: new Date(weatherData.current.dt * 1000).formatTime(
          timezone
        ),
        location,
        sunrise: new Date(weatherData.current.sunrise * 1000).formatTime(
          timezone
        ),
        sunset: new Date(weatherData.current.sunset * 1000).formatTime(
          timezone
        ),
        temp: weatherData.current.temp,
        feelsLike: weatherData.current.feels_like,
        condition: weatherData.current.weather[0].description.toTitleCase(),
        forecast: weatherData.daily[0].weather[0].description.toTitleCase(),
      };
